...
CLAIMS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "claim": "A frozen, pinned-commit corpus of pre-existing repositories (addressed by exact 40-character commit SHA across GitHub, GitLab, and Bitbucket) is scanned and reduced to a single byte-deterministic dataset: a headline statistic (the share of telemetry-bearing repositories that cannot answer 'why did this request fail?'), per-bug-class prevalence with per-host breakdowns, and the diagnosability-score distribution. Each bug class has a precise, testable definition and an explicit soundness/incompleteness statement, and the headline uses the conservative finding-based correlation check so it never over-reports relative to the per-class prevalence. The corpus is grown by a reproducible, user-runnable curation pipeline that discovers telemetry-bearing repositories via code search, clones each candidate into a gitignored directory, pins it to its real HEAD commit SHA (never a fabricated SHA), and keeps only those verified to ship parseable telemetry — yielding a committed corpus of 50+ verified subjects across 3 hosts, tiered so a tiny multi-host subset runs in CI. Re-running is incremental and resumable via a content-addressed cache (keyed by subject SHA, a fail-closed engine-source fingerprint, and the scan options) whose cached and freshly-computed datasets are byte-identical; correlation breakdowns by detected language and instrumentation library and dependency-free deterministic SVG plots accompany the dataset.",
        "public_artifacts": ["telemetry_contracts/bug_classes.py", "telemetry_contracts/mining/__init__.py", "telemetry_contracts/mining/corpus.py", "telemetry_contracts/mining/study.py", "telemetry_contracts/mining/cache.py", "telemetry_contracts/mining/runner.py", "telemetry_contracts/mining/characteristics.py", "telemetry_contracts/mining/correlate.py", "telemetry_contracts/mining/plots.py", "telemetry_contracts/mining/discover.py", "telemetry_contracts/mining/curate.py", "scripts/download_corpus.sh", "scripts/curate_corpus.sh", "docs/evaluation/corpus_study.md"],
        "tests": ["tests/test_mining_study.py", "tests/test_corpus_mining_real.py", "tests/test_corpus_cache.py", "tests/test_corpus_characteristics.py", "tests/test_corpus_runner.py", "tests/test_corpus_discover.py", "tests/test_corpus_curate.py", "tests/test_corpus_manifests.py"],
        "fixtures": ["benchmarks/corpus/tier1.json", "benchmarks/corpus/tier2.json", "benchmarks/corpus/corpus.json"],
        "benchmark_rows": [],
        "limitations": [
