...
- Added first-class OTLP JSON/JSONL import coverage for OpenTelemetry collector-style spans, logs, metrics, histogram exemplars, span links/events/status, resource/scope metadata, source JSON provenance, alias normalization, and importer diagnostics. `examples/otlp/collector_mixed_signals.*`, `examples/otlp/collector_stream_aliases.*`, and `examples/benchmarks/partial_otlp.diagnostics.json` are checked-in validation fixtures; the built-in benchmark reports the labeled partial-import diagnostic and a bounded import loss rate.
