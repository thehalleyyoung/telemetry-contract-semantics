...
- Events: 3
- Counts by kind: `{'log': 1, 'metric': 1, 'span': 1}`
- Names by kind: `{'log': ['checkout.payment_authorized'], 'metric': ['checkout.pay.duration'], 'span': ['checkout.pay']}`
- Fields by kind: `{'log': ['log.name', 'message', 'retry', 'service.name', 'tenant_id'], 'metric': ['service.name', 'tenant_id'], 'span': ['cart_id', 'deployment.environment', 'payment_provider', 'service.name', 'tenant_id']}`
- Correlation key counts: `{'span_id': 3, 'trace_id': 3}`
- Timestamp field counts: `{'end_time_ms': 1, 'end_time_unix_nano': 1, 'start_time_unix_nano': 2, 'time_unix_nano': 2, 'timestamp_ms': 3}`
...
### window-0 group 11111111111111111111111111111111

```mermaid
flowchart TD
  e0["span:checkout.pay"]
  e1["metric:checkout.pay.duration"]
  e2["log:checkout.payment_authorized"]
  e0 -->|exemplar| e1
  e0 -->|log| e2
  e0 -->|hb| e1
  e1 -->|hb| e2
